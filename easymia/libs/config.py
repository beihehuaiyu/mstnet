...
class Config(object):
    ...
    def __init__(self,
        ...
        self.update(
            learning_rate=learning_rate, batch_size=batch_size, epochs=epochs, resume_model=resume_model)
    ...
    def update(self,
        ...
        if batch_size:
            if isinstance(batch_size, int):
                ...
                self.dic['val_batch_size'] = batch_size
                ...
                self.dic['val_batch_size'] = int(infer_batch_size)
            else:
    ...
    def batch_size(self) -> int:
        return self.train_batch_size, self.val_batch_size
    ...
    @property
    def val_config(self) -> Dict:
        return self.dic.get('val', {}).copy()
    ...
    def train_dataset(self) -> paddle.io.Dataset:
        _train_dataset = self.train_config['datasets']
        val_ratio = self.dic.get('val_ratio')
        if not _train_dataset:
            return None
        elif val_ratio is not None:
            _train_dataset['val_ratio'] = val_ratio
            _train_dataset['split'] = 'train'
        else:
            _train_dataset['split'] = 'train'
    ...
    def val_dataset(self) -> paddle.io.Dataset:
        _val_dataset = self.val_config['datasets']
        val_ratio = self.dic.get('val_ratio')
        if not _val_dataset:
            return None
        elif val_ratio is not None:
            _val_dataset['val_ratio'] = val_ratio
            _val_dataset['split'] = 'val'
        else:
            _val_dataset['split'] = 'val'
    ...
    def test_dataset(self) -> paddle.io.Dataset:
        _test_dataset = self.test_config['datasets']

        if not _test_dataset:
            return None
        else:
            _test_dataset['split'] = 'test'
        return self._load_object(_test_dataset)

    @property
    def train_type(self) -> Dict:
        return self.dic.get('train_type', {}).copy()

    def _load_component(self, com_name):
        # 需要mode参数的模块
        com_list_mode = [
            manager.MODELS, manager.TRANSFORMS, manager.LOSSES
        ]

        # 不需要mode参数的模块
        com_list_general = [
            manager.SCHEDULES, manager.METRICS, manager.PLUGINS, manager.TYPES, manager.DATASETS
        ]
